...
### 4.1 OS ごとの慣習の尊重
- ショートカットキーの実装には、JavaFX の `KeyCombination.SHORTCUT_DOWN` を使用してください。
- これにより、macOS では `Command` キー、Windows や Linux では `Control` キーが自動的に割り当てられ、各 OS の標準的なユーザー体験を損なわないようにします。
- 特定の OS でのみ動作するキーバインドを定義する場合は、`util` パッケージなどで OS 判定を行い、適切に分岐させてください。
